...
import { fetchModels, fetchCategories, fetchTags } from '../services/api';
import type { Model3D, Tag } from '../types';
import ModelCard from '../components/ModelCard';
import { SlidersHorizontal, ChevronRight } from 'lucide-react';

const ALL_CATEGORIES = ['All', 'Education', 'Art', 'Gadgets', 'Household', 'Tools', 'Toys & Games', 'Mechanical', 'Miniatures'];
        ...
        <div className="flex items-center gap-4 text-sm">
          {(['newest', 'downloads', 'likes'] as const).map(s => (
            <button
              key={s}
              onClick={() => setSort(s)}
              className={`capitalize transition-colors ${sort === s ? 'text-accent font-semibold' : 'text-text-secondary hover:text-text-primary'}`}
            >
              {s}
            </button>
          ))}
        ...
        {showFilters && (
          <div className="w-[250px] shrink-0 hidden md:block">
            <div className="bg-bg-secondary rounded-xl border border-border p-4">
              <h3 className="text-xs font-semibold text-text-secondary uppercase tracking-wider mb-3">Categories</h3>
                ...
                {ALL_CATEGORIES.map(cat => (
                  <button
                    ...
                    className={`flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-colors ${
                      activeCategory === cat ? 'text-accent bg-accent/10' : 'text-text-secondary hover:text-text-primary'
                    }`}
                  >
                    {cat}
              ...
              {tags.length > 0 && (
                <>
                  <h3 className="text-xs font-semibold text-text-secondary uppercase tracking-wider mt-5 mb-3">Tags</h3>
